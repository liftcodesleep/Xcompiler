...
public class DrawVisitor extends ASTVisitor {
  ...
  public DrawVisitor(int[] nodeCount) {
    this.nodeCount = nodeCount;
    progress = new int[nodeCount.length];
    width = max(nodeCount) * (nodew + horizSep);
    height = nodeCount.length * (nodeh + vertSep);
  ...
  public void draw(String s, AST t) {
    ...
    int x = width / 2 + progress[depth] * hstep - nodeCount[depth] *
        hstep / 2;
    int y = depth * vstep;
  ...
  public Object visitScientificTree(AST t) {
    draw("Scientific: " + ((ScientificTree) t).getSymbol().toString(),
        t);
    return null;
  }
